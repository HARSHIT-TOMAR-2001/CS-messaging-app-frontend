...
export default function ThreadModal({ open, setOpen, selectedThread }) {
//   const handleOpen = () => setOpen(true);
  ...
  const getMessages = async () => {
    try {
      const response = await axios.get(`https://css-messaging-app.herokuapp.com/api/user/thread/messages?threadId=${selectedThread}`, {
        headers: {
  ...
  const handleSend = async () => {
    try {
        ...
        const response = await axios.post(`https://css-messaging-app.herokuapp.com/api/user/create/message`,data, {
          headers: {
              Authorization: JSON.parse(localStorage.getItem("isCSAuth")).authToken
          }
        });
      ...
      >
        <Box sx={style}>
            ...
            <Stack sx={{
            ...
            }} >
                {
                    ...
                    messages.map((msg,index) => {
                        return (
                            <Typography key={index} >{msg.isUserQuery ? "You: ": "Agent: "} {msg.message}</Typography>
                        )
                    })
                    : "Loading..."
                }
            ...
            <Stack direction="row" spacing={2} sx={{
                ...
                <Button onClick={handleClosed} variant="contained">Close</Button>
            </Stack>
        </Box>
      </Modal>
    </div>
